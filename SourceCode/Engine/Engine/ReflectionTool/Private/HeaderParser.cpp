...
void HeaderParser::Parse(Type::TypesList &Types)
{
	...
	do
	{
		...
		if (!GetToken(token))
			...
			Console::LogError("'" + token.GetIdentifier() + "': Bad command or expression");
...
bool HeaderParser::CompileDeclaration(Type::TypesList &Types, Token &DelarationToken)
{
	...
	{
		if (m_CurrentDataStructure == NULL)
		{
			Console::LogError("Access specifier detected out of nest");
			return false;
		}
	...
	else if (m_CurrentDataStructure != NULL && DelarationToken.Matches(m_CurrentDataStructure->GetName(), Token::SearchCases::CaseSensitive))
	...
	else if (DelarationToken.Matches(CLOSE_BRACKET, Token::SearchCases::CaseSensitive))
	{
		...
		if (MatchSymbol(SEMI_COLON) && m_CurrentDataStructure != NULL && m_BlockLevel == m_CurrentDataStructure->GetBlockLevel())
